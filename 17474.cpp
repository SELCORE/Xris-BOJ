...
struct SEG_TREE {
    ...
    NODE add(NODE a, NODE b) {
        if (a.max1 == b.max1)
            return {a.max1, max(a.max2, b.max2), a.cnt_max + b.cnt_max, a.sum + b.sum};
    ...
    NODE init(vector<int> &v, int left, int right, int node) {
        if (left == right)
            return tree[node] = {v[left], -INF, 1, v[left]};
    ...
    void update(int left, int right, int k) {
        update(left, right, k, 0, n - 1, 1);
    }

    void update(int left, int right, int k, int nodeLeft, int nodeRight, int node) {
    ...
    int query_max(int left, int right, int nodeLeft, int nodeRight, int node) {
        ...
        int mid = (nodeLeft + nodeRight) / 2;
        return max(
            query_max(left, right, nodeLeft, mid, node * 2),
            query_max(left, right, mid + 1, nodeRight, node * 2 + 1)
        );
    }

    ll query_sum(int left, int right) {
        return query_sum(left, right, 0, n - 1, 1);
    }
    ...
    ll query_sum(int left, int right, int nodeLeft, int nodeRight, int node) {
        ...
        int mid = (nodeLeft + nodeRight) / 2;
        return query_sum(left, right, nodeLeft, mid, node * 2)
             + query_sum(left, right, mid + 1, nodeRight, node * 2 + 1);
    }
};
